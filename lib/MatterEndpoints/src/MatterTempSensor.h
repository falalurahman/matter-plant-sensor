...
public:
    ...
    // Initialize and register the endpoint. temperatureCelsius is the initial reading.
    bool begin(double temperatureCelsius = 0.0);
    void end();

    // Update the measured temperature (°C, precision to 0.01 °C).
    bool setTemperature(double temperatureCelsius);
    double getTemperature() const { return _rawTemperature / 100.0; }

    void operator=(double temperatureCelsius)  { setTemperature(temperatureCelsius); }
...
private:
    bool    _started  = false;
    int16_t _rawTemperature  = 0;   // value × 100  (signed, supports negative temps)
