// MatterInit - Simplified Matter initialization for custom endpoints
// Based on Arduino Matter library structure
...
node_t *deviceNode = nullptr;  // Remove static to make it accessible from MatterSoilSensor
ArduinoMatterInit::matterEventCB ArduinoMatterInit::_matterEventCB = nullptr;
...
static esp_err_t app_attribute_update_cb(
  ...
  // For custom endpoints, priv_data can be used to pass endpoint-specific context
  switch (type) {
...
static void app_event_cb(const ChipDeviceEvent *event, intptr_t arg) {
  ...
  if (ArduinoMatterInit::_matterEventCB != nullptr) {
    ArduinoMatterInit::_matterEventCB(static_cast<matterEvent_t>(event->Type), event);
  }
}

void ArduinoMatterInit::_init() {
...
bool ArduinoMatterInit::isWiFiStationEnabled() {
...
bool ArduinoMatterInit::isWiFiAccessPointEnabled() {
  // Check hardware support (SOC capabilities) AND Matter configuration
#ifdef SOC_WIFI_SUPPORTED
...
bool ArduinoMatterInit::isThreadEnabled() {
...
bool ArduinoMatterInit::isBLECommissioningEnabled() {
...
bool ArduinoMatterInit::isDeviceCommissioned() {
  return chip::Server::GetInstance().GetFabricTable().FabricCount() > 0;
}

bool ArduinoMatterInit::isWiFiConnected() {
  return chip::DeviceLayer::ConnectivityMgr().IsWiFiStationConnected();
}

bool ArduinoMatterInit::isThreadConnected() {
  return chip::DeviceLayer::ConnectivityMgr().IsThreadAttached();
}

bool ArduinoMatterInit::isDeviceConnected() {
  return ArduinoMatterInit::isWiFiConnected() || ArduinoMatterInit::isThreadConnected();
}

void ArduinoMatterInit::decommission() {
  esp_matter::factory_reset();
}

// Global MatterInit Object
ArduinoMatterInit MatterInit;
